...
class PublicationAssignment(unittest.TestCase):
    ...
    def test_add_generator(self):
        ...
        with self.app as c:
            ...
            rv = self.app.post(
                "/add_generator",
                data=dict(name=gen_name, querystring=gen_qstring, group_id=gen_group),
            )
            # then expect a 200...
            ...
            generator_id = self._get_generator_id(
                gen_name, gen_qstring, self.group_id, self.user_id
            )
    ...
    def test_add_generator_missing_name(self):
        ...
        with self.app as c:
            ...
            for element in rv.response:
                assert (
                    "You must provide a generator name, query string and group id"
                    in element
                )
                # There appears to be a second blank entry, so we'll break after assertion
                break
    ...
    def _get_generator_id(self, gen_name, gen_qstring, group_id, user_id):
        ...
        for generator in generators:
            if (
                ...
                and generator.grp_id == group_id
                and generator.usr_id == user_id
                and generator.querystring == gen_qstring
            ):
