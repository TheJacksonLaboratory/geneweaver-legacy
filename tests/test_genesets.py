...
class TestViewGenesets(unittest.TestCase):
    ...
    def test_get_geneset_genes(self):
        ...
        ep_search_page = (
            ep_root
            + "&start=1&gs_len=10"
            + "&search[value]={}".format(self.search_term)
        )
    ...
    def remove_geneset_data(self):
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """delete from extsrc.geneset_value where gs_id = {}""".format(
                    self.gs_id
                )
            )
            cursor.execute(
                """delete from production.geneset where gs_id = {}""".format(self.gs_id)
            )
            cursor.connection.commit()
    ...
    def get_search_term_from_geneset(self):
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """select substring(cast(gsv_source_list as varchar),2,3) 
                          as search_term from extsrc.geneset_value where gs_id = {}""".format(
                    self.gs_id
                )
            )
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                ...
                (
                    self.user_id,
                    0,
                    "Testing Geneset Paging",
                    "TGP",
                    "A GeneSet for Testing Paging",
                    1,
                    0,
                    0,
                    "normal",
