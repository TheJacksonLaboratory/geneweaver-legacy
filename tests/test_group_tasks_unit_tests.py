...
class GroupTasks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not user:
            user = geneweaverdb.register_user(
                self.first, self.last, self.email, "P@ssW0rd1!"
            )
        ...
        if not groups:
            geneweaverdb.create_group(
                "Testing Users Test Group", "Private", self.user_id
            )
            groups = geneweaverdb.get_user_groups(self.user_id)
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """INSERT INTO production.geneset (usr_id, file_id, gs_name, gs_abbreviation,
                              gs_description, gs_count, gs_gene_id_type, gs_created, gs_status)
                              VALUES (%s, %s, %s, %s, %s, %s, %s, NOW(), %s) RETURNING gs_id""",
                (
                    self.user_id,
                    0,
                    "Testing Users Test GeneSet",
                    "TUTGS",
                    "A GeneSet for Testing Users Testing Purposes",
                    0,
                    0,
                    "normal",
                ),
        ...
        curation_assignments.submit_geneset_for_curation(
            self.geneset_id, self.group_id, "Assignement made in test setup", False
        )
    ...
    def tearDown(self):
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """DELETE from production.notifications where usr_id = %s""",
                (self.user_id,),
            )
            cursor.connection.commit()
    ...
    def test_basic_get_grouptasks(self):
        ...
        curation_assignment_id = curation_assignments.get_geneset_curation_assignment(
            self.geneset_id
        )
    ...
    def _test_grouptasks_with_pubassign(self):
        ...
        pub_assignments.queue_publication(
            publication.pub_id, self.group_id, "Pub Assignment made in group tasks test"
        )
        publication_assignment = pub_assignments.get_publication_assignment_by_pub_id(
            publication.pub_id, self.group_id
        )
        # and publication assigned to a curator
        pub_assignments.assign_publication(
        ...
        new_geneset_id = pub_assignments.create_geneset_stub_for_publication(
            publication_assignment.id,
            "GroupTasks Test Method Geneset",
            "GTMG",
            "This is a geneset stub created in a unit test",
            1,
        )  # species id for mouse
        # When we fetch tasks for a given group
        arguments = MultiDict(
            [
                ("group_id", self.group_id),
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """DELETE FROM production.gs_to_pub_assignment WHERE gs_id = %s
                              and pub_assign_id = %s""",
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """DELETE FROM production.geneset WHERE gs_id = %s""", (new_geneset_id,)
            )

        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """DELETE FROM production.pub_assignments WHERE id = %s""",
                (publication_assignment.id,),
            )
            cursor.connection.commit()
        ...
        with geneweaverdb.PooledCursor() as cursor:
            cursor.execute(
                """SELECT * FROM production.pub_assignments WHERE pub_id = %s""",
                (publication.pub_id,),
            )
            other_pubs = list(geneweaverdb.dictify_cursor(cursor))
            if len(other_pubs) == 0:
                cursor.execute(
                    """DELETE FROM production.publications WHERE pub_id = %s""",
                    (publication.pub_id,),
                )
                cursor.connection.commit()
