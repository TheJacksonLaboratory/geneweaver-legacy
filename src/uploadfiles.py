...
def create_temp_geneset():
    ...
    try:
        with db.PooledCursor() as cursor:
            ...
            cursor.execute(
                """CREATE TABLE IF NOT EXISTS production.temp_geneset_meta (gs_id bigint, sp_id int, gdb_id int)"""
            )
...
def insertPublication(pubDict):
    ...
    if pubDict["pub_pubmed"] != "":
        with db.PooledCursor() as cursor:
            cursor.execute(
                """SELECT pub_id FROM publication WHERE pub_pubmed=%s""",
                (pubDict["pub_pubmed"],),
            )
    ...
    if pmid or pubDict["pub_pubmed"] == "":
        with db.PooledCursor() as cursor:
            cursor.execute(
                """INSERT INTO publication (pub_authors, pub_title, pub_abstract, pub_journal, pub_volume,
...
def insert_new_contents_to_file(contents):
    with db.PooledCursor() as cursor:
        cursor.execute(
            """INSERT INTO file (file_contents, file_comments, file_created) VALUES (%s, %s, now())
                          RETURNING file_id""",
            (
                contents,
...
def insert_into_hom2geneset_by_gsid(gs_id):
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT hom_id FROM homology h INNER JOIN geneset_value gv ON h.ode_gene_id=gv.ode_gene_id 
                        WHERE gv.gs_id=%s""",
            (gs_id,),
        )
        if cursor.rowcount != 0:
            for r in cursor.fetchall():
                cursor.execute(
                    """UPDATE hom2geneset SET geneset_array = geneset_array || %s::bigint 
                                WHERE hom_id = %s""",
                    (
                        gs_id,
                        r[0],
...
def create_new_geneset_for_user(args, user_id):
    ...
    formData = urllib.parse.parse_qs(
        urllib.parse.urlparse(urlString).query, keep_blank_values=True
    )
    ...
    try:
        with db.PooledCursor() as cursor:
            cursor.execute(
                """SELECT production.create_geneset2(%s, %s, %s, %s, %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s)""",
                (
                    int(user_id),
                    int(cur_id),
            ...
            if pub_id is not None:
                cursor.execute(
                    """UPDATE geneset SET pub_id=%s WHERE gs_id=%s""",
                    (
                        pub_id,
                        gs_id,
                    ),
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT count(*) FROM extsrc.geneset_value WHERE gs_id=%s""", (gs_id,)
        )
        if cursor.fetchone()[0] < 1:
            cursor.execute(
                """UPDATE geneset SET gs_status='deleted' WHERE gs_id=%s""", (gs_id,)
            )
            cursor.connection.commit()
            error_string = (
...
def create_new_large_geneset_for_user(args, user_id):
    ...
    try:
        with db.PooledCursor() as cursor:
            cursor.execute(
                """SELECT production.create_geneset_for_queue(%s, %s, %s, %s, %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s)""",
                (
                    int(user_id),
                    int(cur_id),
    ...
    tc.celery_app.send_task(
        "tools.LargeGenesetUpload.LargeGenesetUpload",
        kwargs={"gs_id": gs_id, "user_id": user_id},
    )
...
def create_temp_geneset_from_value(gsid):
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT gs_id FROM temp_geneset_value WHERE gs_id=%s""", (gsid,)
        )
        is_geneset_value = cursor.rowcount
        cursor.execute(
            """SELECT gs_id FROM temp_geneset_meta WHERE gs_id=%s""", (gsid,)
        )
        is_geneset_meta = cursor.rowcount
        if is_geneset_value == 0 and is_geneset_meta == 0:
            with db.PooledCursor() as cursor:
                cursor.execute(
                    """DELETE FROM temp_geneset_value WHERE gs_id=%s""", (gsid,)
                )
                cursor.execute(
                    """INSERT INTO temp_geneset_value (SELECT gs_id, ode_gene_id, gsv_value_list[1], gsv_source_list[1]
                                FROM geneset_value WHERE gs_id=%s)""",
                    (gsid,),
                )
                cursor.connection.commit()
    return
...
def get_file_contents_by_gsid(gsid):
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT file.file_contents FROM file, geneset WHERE file.file_id=geneset.file_id AND
                              geneset.gs_id=%s""",
            (gsid,),
        )
    ...
    if results[0] is None:
        ...
        with db.PooledCursor() as cursor:
            cursor.execute(
                """SELECT f.file_contents, g.sp_id, g.gs_gene_id_type FROM file f, geneset g WHERE g.gs_id=%s
...
def get_unmapped_ids(gs_id, geneset, sp_id, gdb_id):
    ...
    if gdb_id < 0:
        for i in geneset.geneset_values:
            with db.PooledCursor() as cursor:
                cursor.execute(
                    """SELECT ode_ref_id FROM extsrc.gene WHERE ode_gene_id=%s AND sp_id=%s AND gdb_id=-%s""",
                    (
                        i.ode_gene_id,
                        sp_id,
                        gdb_id,
...
def get_temp_geneset_gsid(gsid):
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT sp_id, gdb_id FROM temp_geneset_meta WHERE gs_id=%s""", (gsid,)
        )
        if cursor.rowcount != 0:
            results = cursor.fetchall()
...
def update_species_by_gsid(rargs):
    ...
    if u.is_admin or u.is_curator or g:
        ...
        with db.PooledCursor() as cursor:
            ...
            cursor.execute(
                """INSERT INTO temp_geneset_meta (gs_id) SELECT %s WHERE NOT EXISTS (SELECT gs_id FROM
                              temp_geneset_meta WHERE gs_id=%s)""",
                (
                    gs_id,
                    gs_id,
                ),
            ...
            cursor.execute(
                """UPDATE temp_geneset_meta SET sp_id=%s WHERE gs_id=%s""",
                (
                    sp_id,
                    gs_id,
                ),
            ...
            cursor.execute(
                """UPDATE geneset SET sp_id=%s WHERE gs_id=%s""", (sp_id, gs_id)
            )
            cursor.connection.commit()
            cursor.connection.commit()
            return {"error": "None"}
...
def update_identifier_by_gsid(rargs):
    ...
    if u.is_admin or u.is_curator or g:
        ...
        with db.PooledCursor() as cursor:
            ...
            cursor.execute(
                """UPDATE temp_geneset_meta SET gdb_id=%s WHERE gs_id=%s""",
                (
                    gdb_id,
                    gs_id,
                ),
                    ...
                    for newline in newlines:
                        if re.match(r"#.*$", newline):
                            ...
                            if gene != "" and isinstance(v, float) and gene is not None:
                                ...
                                with db.PooledCursor() as cursor:
                                    cursor.execute(
                                        "SELECT ode_gene_id FROM gene WHERE LOWER(ode_ref_id) LIKE LOWER(%s) "
                                        "AND sp_id=%s AND gdb_id=%s",
                                        (
                                            g,
                                            species,
                                            gdb,
                                ...
                                if len(s) != 0:
                                    for i in s:
                                        with db.PooledCursor() as cursor:
                                            cursor.execute(
                                                """INSERT INTO production.temp_geneset_value
                                                              VALUES (%s, %s, %s, %s)""",
                                                (
                                                    gs_id,
                                                    i[0],
...
def make_file_content_string_from_temp_geneset(gsid):
    ...
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT src_id, src_value FROM temp_geneset_value WHERE gs_id=%s""",
            (gsid,),
        )
        results = cursor.fetchall()
        for i in results:
...
def write_file_contents_to_file(gsid, contents):
    with db.PooledCursor() as cursor:
        cursor.execute(
            """UPDATE file SET file_contents=%s FROM geneset
                          WHERE geneset.gs_id=%s AND geneset.file_id=file.file_id""",
            (
                contents,
                gsid,
            ),
...
def update_geneset_species(gsid):
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM (SELECT sp_id FROM geneset WHERE gs_id=%s UNION
                          SELECT sp_id FROM temp_geneset_meta WHERE gs_id=%s) as a""",
            (
                gsid,
                gsid,
            ),
        ...
        if cursor.rowcount != 1:
            cursor.execute(
                """UPDATE geneset gs SET sp_id=t.sp_id FROM temp_geneset_meta t
                              WHERE gs.gs_id=t.gs_id AND t.gs_id=%s""",
                (gsid,),
            )
            return
        else:
...
def update_geneset_identifier(gsid):
    with db.PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM (SELECT gs_gene_id_type FROM geneset WHERE gs_id=%s UNION
                          SELECT gdb_id FROM temp_geneset_meta WHERE gs_id=%s) as a""",
            (
                gsid,
                gsid,
            ),
        ...
        if cursor.rowcount != 1:
            cursor.execute(
                """UPDATE geneset gs SET gs_gene_id_type=t.gdb_id FROM temp_geneset_meta t
                              WHERE gs.gs_gene_id_type=t.gdb_id AND t.gs_id=%s""",
                (gsid,),
            )
            return
        else:
...
def insert_into_geneset_value_by_gsid(gsid):
    ...
    with db.PooledCursor() as cursor:
        ...
        cursor.execute(
            """select count(*) from production.temp_geneset_value where gs_id = %s;"""
            % (gsid,)
        )
        gs_count = cursor.fetchone()[0]
        cursor.execute(
            """SELECT gs_id FROM production.temp_geneset_value WHERE gs_id=%s""",
            (gsid,),
        )
        g = cursor.fetchone()
        if g is not None:
            if int(g[0]) == int(gsid):
                with db.PooledCursor() as cursor:
                    cursor.execute(
                        """DELETE FROM extsrc.geneset_value WHERE gs_id=%s;""" % (gsid,)
                    )
                    cursor.execute(
                        """INSERT INTO extsrc.geneset_value (gs_id, ode_gene_id, gsv_value, gsv_hits, gsv_source_list,
                    ...
                    cursor.execute(
                        """DELETE FROM production.temp_geneset_value WHERE gs_id=%s;"""
                        % (gsid,)
                    )
                    cursor.execute(
                        """DELETE FROM production.temp_geneset_meta WHERE gs_id=%s;"""
                        % (gsid,)
                    )
                    ...
                    cursor.execute(
                        """UPDATE geneset SET gs_status='normal' WHERE gs_id=%s;""",
                        (gsid,),
                    )
                    cursor.connection.commit()
                    # update the main geneset table with the new count
                    cursor.execute(
                        """update production.geneset set gs_count = %s where gs_id = %s;"""
                        % (gs_count, gsid)
                    )
                    cursor.connection.commit()
...
def post_geneset_by_user(apikey, data):
    ...
    if "file_url" in jData:
        ...
        if response.ok:
            ...
            file_text = (
                file_url_text
                if "file_text" not in jData
                else file_text + "\r" + file_url_text
            )
