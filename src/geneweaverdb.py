...
# New code for Tools, Next 5 functions Modify usr2gene for ABBA
...
def get_gene_and_species_info(ode_ref_id):
    with PooledCursor() as cursor:
        cursor.execute(
            '''SELECT gene.*, species.* FROM extsrc.gene JOIN odestaticspecies WHERE lower(ode_ref_id)=lower(%s);''', (ode_ref_id,))
    return list(dictify_cursor(cursor))
# end block of abba functions
