...
def get_all_members_of_group(usr_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            ...
            (SELECT grp_id FROM usr2grp WHERE usr_id=%s) AND u.usr_id=u2g.usr_id ORDER BY u.usr_email""",
            (usr_id,),
        )
...
def get_group_members(grp_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            ...
            AND u.usr_id=u2g.usr_id ORDER BY u.usr_last_name, u.usr_first_name, u.usr_email""",
            (grp_id,),
        )
...
def get_group_admins(grp_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            ...
            (SELECT usr_id FROM usr2grp WHERE u2g_privileges=1 AND grp_id=%s) ORDER BY usr_email""",
            (grp_id,),
        )
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM production.grp WHERE grp_id in (SELECT grp_id
               ...
               WHERE usr_id = %s and u2g_privileges = 1)""",
            (usr_id,),
        )
...
def get_all_member_groups(usr_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM production.grp WHERE grp_id in (SELECT grp_id
               FROM production.usr2grp
               WHERE usr_id = %s and (u2g_privileges = 0 or u2g_privileges IS NULL))""",
            (usr_id,),
        )
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM production.grp WHERE grp_id NOT IN (SELECT grp_id
               FROM production.usr2grp WHERE usr_id = %s) AND grp_private = false""",
            (usr_id,),
        )
...
def edit_group_name(group_name, group_id, group_private, user_id):
    if int(flask.session["user_id"]) == int(user_id):
        ...
        with PooledCursor() as cursor:
            cursor.execute(
                """UPDATE production.grp SET grp_name=%s, grp_private=%s WHERE grp_id=%s
...
def remove_user_from_group(group_name, owner_id, usr_email):
    with PooledCursor() as cursor:
        ...
        notifications.send_usr_notification(
            usr_email,
            "Removed from Group",
            "You have been removed from the group {} by {}".format(
            ...
            with PooledCursor() as cursor:
                ...
                if cursor.rowcount:
                    notifications.send_usr_notification(
                        ...
                        "You have been promoted to admin of the group {} by {}".format(
        ...
        with PooledCursor() as cursor:
            ...
            if cursor.rowcount:
                notifications.send_usr_notification(
                    ...
                    "You now have standard group membership.".format(
                        group_name, admin_name
                    ),
                )
    ...
    with PooledCursor() as cursor:
        ...
        for r in results:
            ...
            if grp_id in h:
                ...
                cursor.execute(
                    """UPDATE project SET pj_groups=%s WHERE pj_id=%s""",
                    (
                        res,
                        r[1],
                    ),
...
def get_all_species():
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT sp_id, sp_name FROM species WHERE sp_name != '' ORDER BY sp_id;"""
        )
...
def get_all_species_resources():
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT sp_name, sp_taxid, sp_date FROM species WHERE sp_name != '' ORDER BY sp_id;"""
        )
        return list(dictify_cursor(cursor))
...
def get_all_publications(gs_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM publication p, geneset g WHERE p.pub_id=g.pub_id AND g.gs_id=%s""",
            (gs_id,),
        )
...
def get_all_attributions():
    ...
    with PooledCursor() as cursor:
        ...
        cursor.execute(
            """SELECT at_id, at_abbrev FROM attribution WHERE at_abbrev IS NOT NULL ORDER BY at_id;"""
        )
        return OrderedDict(cursor)
...
def get_ode_ref_id(value, sp_id):
    ...
    with PooledCursor() as cursor:
        if sp_id == "None":
            cursor.execute(
                """SELECT ode_ref_id FROM gene WHERE lower(ode_ref_id) LIKE '%s%%' ORDER BY ode_ref_id LIMIT 100"""
                % (value,)
            )
        else:
            cursor.execute(
                """SELECT ode_ref_id FROM gene WHERE lower(ode_ref_id) LIKE '%s%%' AND sp_id=%s ORDER BY ode_ref_id LIMIT 100"""
                % (
                    value,
                    sp_id,
                )
...
def delete_project_by_id(rargs):
    ...
    if (
        user_id != 0
        or get_user(user_id).is_admin is not False
        or get_user(user_id).is_curator is not False
    ):
        with PooledCursor() as cursor:
            cursor.execute(
                """DELETE from project2geneset WHERE pj_id in (%s)"""
                % ",".join(str(x) for x in projids)
            )
            cursor.execute(
                """DELETE from project WHERE pj_id in (%s)"""
                % ",".join(str(x) for x in projids)
            )
...
def add_project_by_name(name, comment):
    ...
    if name == "":
        ...
        if (
            ...
            with PooledCursor() as cursor:
                cursor.execute(
                    """INSERT INTO project (usr_id, pj_name, pj_created, pj_notes) VALUES
                                (%s, %s, now(), %s)""",
                    (
                        user_id,
                        name,
            ...
            return {
                "error": "You do not have permission to add a Project to this account."
            }
...
def change_project_by_id(rargs):
    ...
    if name == "":
        ...
        if (
            ...
            with PooledCursor() as cursor:
                cursor.execute(
                    """UPDATE project SET pj_name=%s, pj_notes=%s WHERE pj_id=%s""",
                    (
                        name,
                        notes,
                        id,
...
def delete_geneset_value_by_id(rargs):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """DELETE from temp_geneset_value WHERE gs_id=%s AND src_id =%s""",
            (
                gs_id,
                gene_id,
            ),
...
def user_is_owner(usr_id, gs_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT COUNT(gs_id) FROM geneset WHERE usr_id=%s AND gs_id=%s""",
            (usr_id, gs_id),
        )
        return cursor.fetchone()[0]
...
def user_is_assigned_curation(usr_id, gs_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT COUNT(gs_id) FROM curation_assignments WHERE curator=%s AND gs_id=%s AND curation_state=%s""",
            (usr_id, gs_id, CurationAssignment.ASSIGNED),
        )
...
def edit_geneset_id_value_by_id(rargs):
    ...
    user_id = flask.session["user_id"]
    if (
        get_user(user_id).is_admin != "False" or get_user(user_id).is_curator != "False"
    ) or user_can_edit(user_id, gs_id):
        with PooledCursor() as cursor:
            cursor.execute(
                """UPDATE temp_geneset_value SET src_id=%s, src_value=%s WHERE gs_id=%s AND src_id=%s""",
                (
                    gene_id,
                    value,
                    gs_id,
...
def update_geneset_values(gs_id):
    ...
    with PooledCursor() as cursor:
        ...
        sql = "select src_value, src_id from production.temp_geneset_value where gs_id = {}".format(
            gs_id
        )
        cursor.execute(sql)
        for row in cursor:
        ...
        udsql = (
            "update production.file set file_contents = '{}' where file_id={}".format(
...
def existing_identifiers_for_geneset(gs_id, gene_ids):
    ...
    with PooledCursor() as c:
        c.execute(
            """SELECT src_id FROM temp_geneset_value WHERE gs_id=%s AND src_id IN %s""",
            (gs_id, gene_ids),
        )
        return c.fetchall()
        ...
        try:
            for row in geneset_row_list:
                ...
                try:
                    ...
                    errors.append(
                        {
                            "error": "There seems to be a problem with {0}. Are they tab delimited with gene names and values?".format(
                                gene
                            )
                        }
                    )
            ...
            results.append(
                {
                    "success": "Genes succesfully uploaded. Please check the results,"
            ...
            errors.append(
                {"error": "Check your upload data for typos or errors and try again."}
            )
        except psycopg2.Error:
            errors.append(
                {"error": "If the problem persists please contact Geneweaver support."}
            )
...
def add_geneset_gene_to_temp(gs_id, user_id, gene_id, value, overwrite=False):
    ...
    if (
        ...
        with PooledCursor() as cursor:
            ...
            cursor.execute(
                """SELECT g.ode_gene_id FROM gene g, geneset gs WHERE gs.gs_id=%s AND gs.sp_id=g.sp_id AND
							  g.ode_ref_id=%s""",
                (
                    gs_id,
                    gene_id,
            ...
            if cursor.rowcount == 0:
                return {
                    "error": "Identifier Not Found for ID: {}, Value: {}".format(
                        gene_id, value
                    )
                }
            else:
            ...
            if not overwrite:
                ...
                cursor.execute(
                    """SELECT src_id FROM temp_geneset_value WHERE gs_id=%s AND src_id=%s""",
                    (
                        gs_id,
                        gene_id,
                    ),
                ...
                if cursor.rowcount != 0:
                    return {
                        "error": "The Source Identifier ({}) already exists for this geneset".format(
                            gene_id
                        )
                    }
            ...
            cursor.execute(
                """INSERT INTO temp_geneset_value (gs_id, ode_gene_id, src_value, src_id)
							  VALUES (%s, %s, %s, %s)""",
                (
                    gs_id,
                    ode_gene_id,
...
def cancel_geneset_edit_by_id(rargs):
    ...
    if (
        ...
        with PooledCursor() as cursor:
            cursor.execute(
                """DELETE FROM temp_geneset_value WHERE gs_id=%s""", (gs_id,)
            )
...
def update_geneset(usr_id, form):
    ...
    if (
        (get_user(usr_id).is_admin == False and get_user(usr_id).is_curator == False)
        and (user_is_owner(usr_id, gs_id) != 1)
        and not user_is_assigned_curation(usr_id, gs_id)
    ):
        return {
            "success": False,
    ...
    if not gs_abbreviation or not gs_description or not gs_name:
        return {"success": False, "error": "You did not provide a required field"}
...
def user_is_project_owner(user_id, proj_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT COUNT(pj_id) FROM project WHERE usr_id=%s AND pj_id=%s""",
            (user_id, proj_id),
        )
        return cursor.fetchone()[0]
...
def remove_geneset_from_project(rargs):
    ...
    if get_user(user_id).is_admin or user_is_project_owner(user_id, proj_id):
        with PooledCursor() as cursor:
            cursor.execute(
                """DELETE FROM project2geneset WHERE pj_id=%s AND gs_id=%s""",
                (
                    proj_id,
                    gs_id,
                ),
...
def update_stared_project(proj_id, user_id):
    if int(user_id) == int(flask.session["user_id"]):
        with PooledCursor() as cursor:
            ...
            cursor.execute(
                """UPDATE project SET pj_star=%s WHERE pj_id=%s""",
                (
                    star,
                    proj_id,
                ),
...
def update_threshold_values(rargs):
    ...
    if (
        (
            get_user(user_id).is_admin != "False"
            or get_user(user_id).is_curator != "False"
        )
        or user_is_owner(user_id, gs_id) != 0
        or user_is_assigned_curation(user_id, gs_id)
    ):
        minmax = str(min) + "," + str(max)
        with PooledCursor() as cursor:
            cursor.execute(
                """UPDATE geneset SET gs_threshold_type=5, gs_threshold=%s WHERE gs_id=%s""",
                (
                    minmax,
                    gs_id,
                ),
...
def get_server_side_genesets(rargs):
    ...
    source_columns = [
        "cast(sp_id as text)",
        "cast(cur_id as text)",
        "cast(gs_attribution as text)",
        "cast(gs_count as text)",
        "cast(gs.gs_id as text)",
        "cast(gs_name as text)",
    ]
    ...
    limit_clause = (
        "LIMIT %d OFFSET %d" % (iDisplayLength, iDisplayStart)
    ...
    if search_value:
        for i in range(len(source_columns)):
            search_clauses.append(
                """%s LIKE '%%%s%%' """ % (source_columns[i], search_value)
            )
    ...
    sort_dir = "ASC NULLS LAST" if sorting_direction == "asc" else "DESC NULLS LAST"
    ...
    # print where_clause
    sql = " ".join([select_clause, where_clause, order_clause, limit_clause]) + ";"
...
def get_server_side_grouptasks(rargs):
    ...
    if group_id:
        ...
        search_columns = [
            "cast(uc.usr_first_name as text)",
        ...
        union_columns = [
            "cast(uc1.usr_first_name as text)",
        ...
        if "status" == search_field:
            ...
            search_clauses = [
                "LOWER({0}) LIKE LOWER('%{1}%') ".format(
                    search_columns[i], search_value
                )
                for i in range(0, 2)
            ]
            union_clauses = [
                "LOWER({0}) LIKE LOWER('%{1}%') ".format(union_columns[i], search_value)
                for i in range(0, 2)
            ]
            search_clause = "OR ".join(search_clauses)
            ...
            operator, date_value = re.match(
                r"^(<|>|=|<=|>=)?\s*(\d{4}-\d{2}-\d{2})", search_value
            ).groups()
            if not operator:
                operator = "="
            cte_clause = "updated {1} '{2}' ".format(
                search_columns[3], operator, date_value
            )
        ...
        sorting_direction = rargs.get("order[0][dir]", type=str)
        sort_dir = (
            "ASC NULLS FIRST" if sorting_direction == "asc" else "DESC NULLS FIRST"
        )
        order_clause = "ORDER BY %s %s" % (sorting_col, sort_dir) if sorting_col else ""
...
def get_server_side_results(rargs):
    ...
    source_columns = [
        "cast(res_id as text)",
        "cast(res_runhash as text)",
        "cast(res_created as text)",
        "cast(res_name as text)",
...
def get_server_side(rargs):
    ...
    # print where_clause
    sql = (
        " ".join([select_clause, from_clause, where_clause, order_clause, limit_clause])
        + ";"
    )
    # print sql
...
def get_primary_keys(table):
    sql = (
        """SELECT pg_attribute.attname FROM pg_index, pg_class, pg_attribute WHERE pg_class.oid = '%s'::regclass AND indrelid = pg_class.oid AND pg_attribute.attrelid = pg_class.oid AND pg_attribute.attnum = any(pg_index.indkey) AND indisprimary;"""
        % (table)
    )
    try:
        with PooledCursor() as cursor:
...
def get_required_columns(table):
    sql = (
        """SELECT column_name FROM information_schema.columns WHERE table_name='%s'AND table_schema='%s' AND is_nullable='NO' AND column_name NOT IN (SELECT column_name FROM information_schema.columns WHERE table_name = '%s' AND column_default LIKE '%s' AND table_schema='%s');"""
        % (
            table.split(".")[1],
            table.split(".")[0],
            table.split(".")[1],
...
def get_nullable_columns(table):
    sql = (
        """SELECT column_name FROM information_schema.columns WHERE table_name='%s' AND table_schema='%s' AND is_nullable='YES' AND column_name NOT IN (SELECT column_name FROM information_schema.columns WHERE table_name = '%s' AND column_default LIKE '%s' AND table_schema='%s');"""
        % (
            table.split(".")[1],
            table.split(".")[0],
            table.split(".")[1],
...
def get_gdb_id_by_name(gdb_name):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT gdb_id FROM genedb WHERE gdb_shortname=%s""", (gdb_name,)
        )
        if cursor.rowcount != 0:
            results = cursor.fetchone()[0]
            return -1 * results
            ...
            cursor.execute(
                """SELECT pf_id FROM platform WHERE pf_shortname=%s""", (gdb_name,)
            )
            results = cursor.fetchone()[0]
            return results
...
def get_bimodal_threshold(gs_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT gsv_value FROM geneset_value WHERE gs_id=%s GROUP BY gsv_value""",
            (gs_id,),
        )
...
def tool_stats_by_user(user_id):
    try:
        with PooledCursor() as cursor:
            cursor.execute(
                """SELECT res_description, res_started, age(res_completed,res_started) as res_duration
						  FROM production.result
						  WHERE res_created >= now() - interval '30 days' AND usr_id=%s
						  ORDER BY res_started desc;""",
                (user_id,),
            )
        return list(dictify_cursor(cursor))
    except Exception as e:
...
def avg_tool_times(keys, tool):
    sql = (
        """SELECT avg(res_completed - res_started) FROM production.result WHERE res_tool='%s' AND res_id=%s;"""
...
def avg_genes(keys):
    sql = """SELECT avg(gs_count) FROM production.geneset WHERE gs_id=%s;""" % (
        " OR gs_id=".join(str(v) for v in keys)
    )
    # print sql
    try:
        with PooledCursor() as cursor:
...
def delete_usr2gene_by_user(user_id):
    with PooledCursor() as cursor:
        cursor.execute("""DELETE FROM usr2gene WHERE usr_id=%s;""", (user_id,))
...
def delete_usr2gene_by_user_and_gene(user_id, ode_gene_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """DELETE FROM usr2gene WHERE usr_id=%s AND ode_gene_id=%s;""",
            (
                user_id,
                ode_gene_id,
            ),
...
def get_groups_owned_by_user(user_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT g.grp_id AS grp_id, g.grp_name AS grp_name, u.u2g_privileges AS priv FROM grp g, usr2grp u
                          WHERE u.usr_id=%s AND u.u2g_privileges=1 AND g.grp_id=u.grp_id""",
            (user_id,),
        )
...
def get_group_by_id(group_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT g.grp_id AS grp_id, g.grp_name AS grp_name, g.grp_private AS private FROM grp g
                          WHERE g.grp_id=%s""",
            (group_id,),
        )
...
class MSETGeneset:
    ...
    def geneset_values(self):
        if self.__geneset_values is None:
            self.__geneset_values = get_geneset_values_for_mset(
                self.project_id, self.int_id
            )
...
def get_publication_by_pubmed(pubmed_id, create=False):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            "SELECT * from publication WHERE pub_pubmed=CAST(%s as VARCHAR)",
            (pubmed_id,),
        )
        ...
        if len(publications) >= 1:
            ...
            if pub_dict:
                if create:
                    ...
                    sql = (
                        """INSERT INTO publication (%s) VALUES (%s) RETURNING pub_id"""
...
class SRP:
    def __init__(self, srp_dict):
        self.srp_accession = (
            srp_dict["srp_accession"] if "srp_accession" in srp_dict else ""
        )
...
def get_srp(gs_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT p.pub_pubmed FROM publication p, geneset gs WHERE gs.gs_id=%s AND p.pub_id=gs.pub_id""",
            (gs_id,),
        )
        ...
        if len(pmid) > 0:
            ...
            if len(srp) > 0:
                cursor.execute(
                    """INSERT INTO srp_sequence_read_archive(srp_accession, srp_date) SELECT %s, now() WHERE NOT EXISTS(SELECT 1 FROM srp_sequence_read_archive WHERE srp_accession=%s)""",
                    (
                        srp,
                        srp,
                    ),
                ...
                cursor.connection.commit()
                cursor.execute(
                    """SELECT srp_id from srp_sequence_read_archive WHERE srp_accession=%s""",
                    (srp,),
                )
                srp_id = cursor.fetchone()[0]
                cursor.execute(
                    """INSERT INTO g2s_geneset2srp(gs_id,srp_id) SELECT %s, %s WHERE NOT EXISTS(SELECT 1 FROM g2s_geneset2srp WHERE srp_id=%s)""",
                    (
                        gs_id,
                        srp_id,
                        srp_id,
                ...
                cursor.connection.commit()
                cursor.execute(
                    """SELECT s.srp_accession, s.srp_date FROM extsrc.srp_sequence_read_archive s, production.g2s_geneset2srp g WHERE s.srp_id = g.srp_id AND gs_id = %s """,
                    (gs_id,),
                )
...
def update_geneset_date(gs_id):
    with PooledCursor() as cursor:
        cursor.execute(
            """UPDATE geneset SET gs_updated = NOW() WHERE gs_id = %s RETURNING gs_updated""",
            (gs_id,),
        )
        cursor.connection.commit()
...
def get_publication(pub_id):
    with PooledCursor() as cursor:
        cursor.execute("SELECT * from publication WHERE pub_id=%s", (pub_id,))
...
class Geneset:
    ...
    def ontological_associations(self):
        if self.__ontological_associations is None:
            self.__ontological_associations = get_all_ontologies_by_geneset(
                self.geneset
            )
...
def get_user_byemail(user_email):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT * FROM usr WHERE usr_email=%s""", (user_email.lower(),)
        )
        users = [User(row_dict) for row_dict in dictify_cursor(cursor)]
        if len(users) > 1:
...
def register_user_from_guest(
    user_first_name, user_last_name, user_email, user_password, guest_id
):
    """
    ...
    with PooledCursor() as cursor:
        ...
        cursor.execute(
            """UPDATE usr SET
               ...
               WHERE usr.usr_id = %s""",
            (user_first_name, user_last_name, user_email, password_md5, guest_id),
        )
        cursor.connection.commit()
        return get_user_byemail(user_email)
...
def update_notification_pref(user_id, state):
    with PooledCursor() as cursor:
        cursor.execute("""select usr_prefs FROM usr WHERE usr_id=%s""", (user_id,))
        results = cursor.fetchall()
        if len(results) == 1:
            preferences = json.loads(results[0][0])
            preferences["email_notification"] = state
            cursor.execute(
                """UPDATE usr SET usr_prefs=%s WHERE usr_id=%s""",
...
def update_annotation_pref(user_id, annotator):
    if annotator not in ann.ANNOTATORS:
        return {
            "error": "invalid annotator: '{}' must be one of {}".format(
                annotator, ", ".join(ann.ANNOTATORS)
            )
        }
    with PooledCursor() as cursor:
        cursor.execute("""select usr_prefs FROM usr WHERE usr_id=%s""", (user_id,))
...
def get_similar_genesets_by_publication(geneset_id, user_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT gs_id FROM geneset WHERE pub_id IN (SELECT pub_id FROM geneset WHERE gs_id=%s)""",
            (geneset_id,),
        )
        res = cursor.fetchall()
        for r in res:
...
def get_genesets_for_publication(pub_id, user_id):
    ...
    with PooledCursor() as cursor:
        ...
        if gs_ids_readable:
            SQL = "SELECT geneset.* FROM geneset WHERE geneset.gs_id IN ({})".format(
                ",".join(["%s"] * len(gs_ids_readable))
            )
...
def get_groups_by_project(proj_id):
    ...
    with PooledCursor() as cursor:
        ...
        cursor.execute(
            """SELECT g.grp_id, g.grp_name, u.usr_email, g.grp_private FROM grp g, usr u, usr2grp u2g
                          WHERE g.grp_id=u2g.grp_id AND u2g.usr_id=u.usr_id AND g.grp_id IN (%s)"""
...
def get_geneset_hom_ids(gs_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT DISTINCT hom_id FROM extsrc.homology h INNER JOIN extsrc.geneset_value gsv 
                ON h.ode_gene_id=gsv.ode_gene_id INNER JOIN production.geneset g ON gsv.gs_id=g.gs_id 
                WHERE gs_status not like 'de%%' AND g.gs_id=%s""",
            (gs_id,),
        )
        if cursor.rowcount == 0:
            return 0
...
def get_genesets_by_hom_id(hom_ids):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            "SELECT geneset_array FROM extsrc.hom2geneset WHERE hom_id=ANY(%s)",
            (hom_ids,),
        )
...
def get_genesets_hom_ids(gs_ids):
    ...
    with PooledCursor() as cursor:
        ...
        if cursor.rowcount == 0:
            ...
            return {r[0]: list(r[1]) for r in cursor.fetchall()}
...
def insert_into_geneset_jaccard(jaccards, gs_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """DELETE FROM geneset_jaccard WHERE gs_id_left=%s OR gs_id_right=%s""",
            (
                gs_id,
                gs_id,
            ),
        ...
        for key, value in jaccards.items():
            if gs_id > key:
                ...
                gs_right = key
            cursor.execute(
                """INSERT INTO geneset_jaccard (gs_id_left, gs_id_right, jac_value) VALUES (%s, %s, %s)""",
                (
                    gs_left,
                    gs_right,
                    value,
...
def get_all_parents_to_root_for_ontology(ont_id):
    ...
    while result[0] != 0:
        ...
        result = (
            cursor.fetchall()
        )  # Fun fact: cursor.fetchall() returns a list of tuples
        if (
            result[0][0] == 1
        ):  # result[0] = a tuple of one element, result[0][0] = an element from a tuple from a list
...
def get_all_ontologydb():
    ...
    """
    with PooledCursor() as cursor:
        cursor.execute("""SELECT * FROM ontologydb;""")
...
def get_genes_by_gs_id(geneset_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT ode_gene_id FROM geneset_value WHERE gs_id=%s""", (geneset_id,)
        )
        res = cursor.fetchall()
        for r in res:
            genes.append(r[0])
    ...
    sql = """SELECT ode_ref_id FROM gene WHERE gene.ode_gene_id IN
                            (SELECT distinct h2.ode_gene_id FROM homology h1
                                ...
                                WHERE h2.hom_source_name='Homologene'
                                      ...
                                      AND {} IN %(genelist)s)
                            AND sp_id=%(newSpecies)s AND ode_pref='t' AND gdb_id=7;""".format(
        gene_id_type
    )
...
def get_omicssoft(gs_id):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT os_project, os_tag, os_source FROM production.omicsoft WHERE gs_id=%s""",
            (gs_id,),
        )
        res = cursor.fetchall()
        if res is not None:
...
def get_all_geneset_values(gs_id):
    ...
    with PooledCursor() as cursor:
        if geneset.gene_id_type < 0:
            cursor.execute(
                """SELECT gv.gsv_value as gsv, g.ode_ref_id as ref FROM geneset_value gv, gene g, geneset gs WHERE
							  g.ode_pref='t' AND gv.gs_id=%s AND gs.gs_id=gv.gs_id AND gs.sp_id=g.sp_id AND
							  gv.ode_gene_id=g.ode_gene_id ORDER BY gv.gsv_value ASC""",
                (gs_id,),
            )
        else:
            cursor.execute(
                """SELECT gv.gsv_value, g.ode_ref_id FROM geneset_value gv, gene g, geneset gs WHERE
							  gv.gs_id=%s AND gs.gs_id=gv.gs_id AND gs.sp_id=g.sp_id AND gv.ode_gene_id=g.ode_gene_id AND
							  g.ode_pref='t' ORDER BY gv.gsv_value ASC""",
                (gs_id,),
            )
    ...
    with PooledCursor() as cursor:
        if geneset.gene_id_type < 0:
            cursor.execute(
                """SELECT gv.gsv_value as gsv, g.ode_ref_id as ref FROM geneset_value gv, gene g, geneset gs WHERE
							  g.ode_pref='t' AND gv.gs_id=%s AND gs.gs_id=gv.gs_id AND gs.sp_id=g.sp_id AND
							  gv.ode_gene_id=g.ode_gene_id AND g.gdb_id=%s ORDER BY gv.gsv_value ASC""",
                (
                    gs_id,
                    gdb_id,
                ),
            ...
            cursor.execute(
                """SELECT gv.gsv_value, g.ode_ref_id FROM geneset_value gv, gene g, geneset gs WHERE
							  gv.gs_id=%s AND gs.gs_id=gv.gs_id AND gs.sp_id=g.sp_id AND gv.ode_gene_id=g.ode_gene_id AND
							  g.ode_pref='t' AND g.gdb_id=%s ORDER BY gv.gsv_value ASC""",
                (
                    gs_id,
                    gdb_id,
                ),
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            ...
            WHERE gsv.gs_id IN
                  ...
                  END"""
            + s,
            (pj_tg_id, pj_int_id, pj_tg_id, ode_ref),
        )
...
class ToolConfig:
    def __init__(self, tool_dict):
        ...
        try:
            self.requirements = [
                x.strip() for x in tool_dict["tool_requirements"].split(",")
            ]
        except:
    ...
    def params(self):
        if self.__params is None:
            self.__params = OrderedDict(
                ((tp.name, tp) for tp in get_tool_params(self.classname))
            )
...
def get_all_userids():
    with PooledCursor() as cursor:
        (cursor.execute("""SELECT usr_id, usr_email FROM production.usr LIMIT 15;"""),)
    return list(dictify_cursor(cursor))
...
def get_sp_id():
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT sp_id FROM species WHERE sp_id > 0 ORDER BY sp_id ASC;"""
        )
...
def get_gdb_names():
    ...
    gdb_names = []
    with PooledCursor() as cursor:
        cursor.execute("""SELECT gdb_name FROM genedb ORDER BY gdb_id""")
        for name in cursor:
        ...
        cursor.execute(
            """SELECT gsv.ode_gene_id, g.ode_ref_id, gdb.gdb_name 
                FROM gene g, genedb gdb, geneset_value gsv, geneset gs
                 ...
                 AND g.gdb_id=gdb.gdb_id ORDER BY g.ode_gene_id"""
            % (gs_id,)
        )
        results = cursor.fetchall()
        ...
        for gid in results:
            ...
            if str(gid[2]) == name:
                if data[gid[0]][name] == "-":
                    ...
                    data[gid[0]][name] = "|".join(
                        (cur_value + "|" + str(gid[1])).split("|")
                    )
    return data
...
def get_file(apikey, task_id, file_type):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT usr_id FROM production.result WHERE res_runhash=%s""", (task_id,)
        )
...
def get_user_id_by_apikey(apikey):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT usr_id FROM production.usr WHERE apikey=%s""", (apikey,)
        )
    return cursor.fetchone()
...
def get_ontologies_by_ontdb_id(ontdb_id):
    ...
    ontologies = [
        {"id": row_dict["ont_id"], "text": row_dict["ont_name"]}
        for row_dict in dictify_cursor(cursor)
    ]
    return ontologies
...
def get_genesets_by_projects(apikey, projectids):
    ...
    query = (
        "SELECT gs_id FROM production.project2geneset WHERE pj_id IN "
