...
def admin_edit(strdata):  
	...
	table=data[0]
	keyID=data[1]
	columns = geneweaverdb.admin_get_data(table,keyID)
	columns = geneweaverdb.admin_set_edit("")

	print columns[0]

        return AdminEdit().render("admin/adminEdit.html", columns=columns, table=table)
    else:
