...
def landing_page():
    ...
    genes = GetGenesetsByGeneRefIdHomology.get(self=0, apikey=apikey, gene_ref_id='MGI:95929',
                                      gdb_name='MGI')

    sp_ids = []
    for g in genes:
        sp_ids.append(g[0]['sp_id'])

    sp_ids_frequency = {'one':sp_ids.count(1), 'two':sp_ids.count(2), 'three': sp_ids.count(3), 'six':sp_ids.count(6)}
    #sp_ids_frequency = {'one': 1314, 'two': 1733, 'three': 136, 'six': 13}
    return render_template('landing_page.html', gs_counts=gs_counts, sp_ids_frequency=sp_ids_frequency)
