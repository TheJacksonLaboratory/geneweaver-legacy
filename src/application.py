...
admin = Admin(
    app, name="GeneWeaver", index_view=adminviews.AdminHome(url="/admin", name="Admin")
)
...
admin.add_view(
    adminviews.Viewers(
        name="Recent Users", endpoint="viewRecentUsers", category="User Tools"
    )
)
admin.add_view(
    adminviews.Viewers(
...
admin.add_view(
    adminviews.Viewers(name="Groups", endpoint="viewGroups", category="User Tools")
)
admin.add_view(
    adminviews.Viewers(name="Projects", endpoint="viewProjects", category="User Tools")
)
admin.add_view(
    adminviews.Viewers(name="Files", endpoint="viewFiles", category="User Tools")
)

admin.add_view(
    adminviews.Viewers(name="GeneSets", endpoint="viewGenesets", category="Gene Tools")
)
admin.add_view(
...
admin.add_view(
    adminviews.Viewers(name="Gene Info", endpoint="viewGeneInfo", category="Gene Tools")
)
admin.add_view(
    adminviews.Viewers(
...
admin.add_view(
    adminviews.Viewers(name="News", endpoint="viewNewsFeed", category="User Tools")
)
...
admin.add_view(adminviews.Add(name="News Item", endpoint="newNewsItem", category="Add"))
...
def static_results(filename):
    return send_from_directory(config.get("application", "results"), filename)
...
def _logout():
    ...
    params = {
        "returnTo": url_for("render_home", _external=True),
        "client_id": config.get("auth", "client_id"),
    }
...
def sso_login():
    ...
    return auth0.authorize_redirect(
        redirect_uri=redirect_uri, audience=config.get("auth", "audience")
    )
...
def sso_signup():
    return auth0.authorize_redirect(
        redirect_uri=url_for("callback_handling", _external=True), screen_hint="signup"
    )
...
def callback_handling():
    ...
    try:
        ...
        if e.description == "Jax users must use Jax connection to log in.":
            _logout()
            params = {
                "returnTo": url_for("callback_error_jax_email", _external=True),
                "client_id": config.get("auth", "client_id"),
            }
            return redirect(auth0.api_base_url + "v2/logout?" + urlencode(params))
    ...
    try:
        ...
        _logout()
        params = {
            "returnTo": url_for("callback_error_duplicate_email", _external=True),
            "client_id": config.get("auth", "client_id"),
        }
        return redirect(auth0.api_base_url + "v2/logout?" + urlencode(params))
...
def render_analyze():
    ...
    return render_template("analyze.html", active_tools=active_tools, grp2proj=grp2proj)
...
def render_analyze_shared():
    ...
    return render_template(
        "analyze_shared.html", active_tools=active_tools, grp2proj=grp2proj
    )
...
def render_editgenesets(gs_id, curation_view=False):
    ...
    if user_id != 0:
        view = (
            ...
            or user_info.is_curator
            or geneset.user_id == user_id
            or geneweaverdb.user_is_assigned_curation(user_id, gs_id)
            else None
        )
    else:
        view = None
    ...
    for gs_id in gs_ids:
        ...
        if geneset and (user.user_id == geneset.user_id or user.is_admin):
            curation_assignments.submit_geneset_for_curation(
                geneset.geneset_id, grp_id, note
            )
...
def assign_genesets_to_curator():
    ...
    for gs_id in gs_ids:
        ...
        if assignment:
            ...
            status[gs_id] = {
                "success": False,
                "message": "Error assigning curator, GeneSet "
                + str(gs_id)
...
def render_assign_publication(group_id):
    generators = []
    user_id = session["user_id"]
    my_groups = geneweaverdb.get_all_owned_groups(
        user_id
    ) + geneweaverdb.get_all_member_groups(user_id)
    ...
    if not group_id:
        generators = publication_generator.list_generators(
            user_id, [str(group["grp_id"]) for group in my_groups]
        )

    return render_template(
        "publication_assignment.html", myGroups=my_groups, myGenerators=generators
    )
...
def add_generator():
    ...
    if generator_name and generator_querystring and group_id:
        ...
        response = jsonify(
            success=False,
            message="You must provide a generator name, query string and group id.",
        )
        response.status_code = 412
...
def update_generator():
    ...
    if generator_id and generator_name and generator_querystring and group_id:
        ...
        try:
            ...
            response = jsonify(
                message="Generator not saved.  Error encountered while updating database: {0}".format(
                    error
                )
            )
    else:
...
def delete_generator(generator_id, usr_id):
    ...
    if not (flask_user == usr_id):
        ...
        response = jsonify(
            message="You cannot delete a generator belonging to another user."
        )
        response.status_code = 412

    else:
        generator = publication_generator.PublicationGenerator.get_generator_by_id(
            generator_id
        )
        if generator:
...
def run_generator(generator_id):
    ...
    try:
        ...
        return json.dumps(
            {"error": "Problem communicating with PubMed, please try again later..."}
        )
...
def assign_publication_to_group():
    ...
    if group_id not in geneweaverdb.get_user_groups(user_id):
        response = jsonify(
            message="You do not have permissions to assign tasks to this group."
        )
        response.status_code = 403
        ...
        if not publication:
            ...
            publication_assignment = (
                pub_assignments.get_publication_assignment_by_pub_id(
                    publication.pub_id, group_id
                )
            )

            if (
                publication_assignment
                and publication_assignment.state != publication_assignment.REVIEWED
            ):
        ...
        for pubmed_id in pubmed_ids:
            ...
            if not publication:
                ...
                if (
                    ...
                    # everything is good,  do the assignment
                    pub_assignments.queue_publication(
                        publication.pub_id, group_id, notes
                    )
                    status[pubmed_id] = {
                        "success": True,
...
def assign_publication_to_curator():
    ...
    if assignment:
        owned_groups = geneweaverdb.get_all_owned_groups(session["user_id"])
        curator_groups = geneweaverdb.get_all_owned_groups(
            curator
        ) + geneweaverdb.get_all_member_groups(curator)
...
def mark_pub_assignment_as_complete():
    ...
    if assignment:
        if assignment.assignee != user_id:
            ...
            for gs in geneweaverdb.get_genesets_for_publication(
                assignment.pub_id, user_id
            ):
                gs_state = curation_assignments.get_geneset_curation_assignment(
                    gs.geneset_id
                ).state
...
def close_pub_assignment():
    ...
    if assignment:
        if assignment.assigner != user_id:
            ...
            response = jsonify(
                message="The assignment must first be made ready for review."
            )
            response.status_code = 403
        else:
            # check to make sure that all attached geneset assignments are approved
    ...
    if "user_id" in session:
        ...
        if pub_assignment:
            ...
            if uid == pub_assignment.assignee and uid == pub_assignment.assigner:
                ...
                view = "assigner"
            elif pub_assignment.group in [
                g["grp_id"] for g in geneweaverdb.get_all_owned_groups(uid)
            ]:
                view = "group_admin"
            elif uid in [
                u["usr_id"]
                for u in geneweaverdb.get_group_members(pub_assignment.group)
            ]:
                view = "group_member"
            else:
                view = "no_access"
            ...
            if view != "no_access":
                ...
                genesets_for_pub = geneweaverdb.get_genesets_for_publication(
                    pub_assignment.pub_id, uid
                )
                other_genesets = [
                    gs
                    for gs in genesets_for_pub
                    if gs.geneset_id not in [gs.geneset_id for gs in genesets]
                ]
                ...
                for gs in all_genesets:
                    geneset_assignmnet_map[gs.geneset_id] = (
                        curation_assignments.get_geneset_curation_assignment(
                            gs.geneset_id
                        )
                    )
            ...
            if view == "assigner" or view == "group_admin":
                ...
                curation_team_members = [
                    geneweaverdb.get_user(uid)
                    for uid in geneweaverdb.get_group_users(pub_assignment.group)
                ]
        else:
...
def save_pub_assignment_note():
    ...
    if assignment:
        ...
        if (
            uid == assignment.assignee
            or uid == assignment.assigner
            or int(gid) in [g["grp_id"] for g in geneweaverdb.get_all_owned_groups(uid)]
        ):
            # SAVE notes to DB
...
def create_geneset_stub():
    ...
    if assignment.state != assignment.ASSIGNED and assignment.assignee != user_id:
        ...
        gs_ids = [
            assignment.create_geneset_stub(
                stub["gs_name"],
                stub["gs_label"],
                stub["gs_description"],
                species_id,
        ...
        for gs in gs_ids:
            ...
            geneset_assignment_map[gs] = {
                "status_id": assignment.state,
                "status_name": assignment.status_to_string(),
            }
...
def rerun_annotator():
    ...
    if (not user.is_admin and not user.is_curator) and (
        not geneweaverdb.user_is_owner(user_id, gs_id)
        and not geneweaverdb.user_is_assigned_curation(user_id, gs_id)
    ):
        response = jsonify(
...
def get_geneset_annotations():
    ...
    for an in annos:
        ref_type = geneweaverdb.get_geneset_annotation_reference(gsid, an["ont_id"])
...
def init_ont_tree():
    ...
    assoc_annotations = geneweaverdb.get_all_ontologies_by_geneset_and_db(
        gs_id, ontdb_id
    )
    ...
    for annotation in assoc_annotations:
        ...
        paths = geneweaverdb.get_all_parents_to_root_for_ontology(
        ...
        for path in paths:
            ...
            for i in range(0, len(ontpath)):
                if not i == len(ontpath) - 1:
                    ...
                    child_annotations = geneweaverdb.get_all_children_for_ontology(
                        ontpath[i]["key"]
                    )
                    children = list()
                    ...
                    for j in range(0, len(child_annotations)):
                        ...
                        if child_node in [
                            s for s in children if s["title"] == ontpath[i + 1]["title"]
                        ]:
...
def create_new_expanded_child_dict(ontology_node, parents, end_node, grt):
    ...
    if ontology_node.ontology_id in parents:
        if ontology_node.ontology_id == end_node:
            ...
            new_child_dict["children"] = []
            ontology_node_children = geneweaverdb.get_all_children_for_ontology(
                (ontology_node.ontology_id)
            )
            for child in ontology_node_children:
                if child.ontology_id in parents:
                    new_child_dict["children"].append(
                        create_new_expanded_child_dict(child, parents, end_node, grt)
                    )
                else:
...
def render_editgeneset_genes(gs_id, curation_view=False):
    ...
    # for probe sets
    genesnotfound = uploadfiles.get_unmapped_ids(
        gs_id, geneset, geneset.sp_id, geneset.gene_id_type
    )

    if user_id != 0:
        view = (
            "True"
            if user_info.is_admin or user_info.is_curator or geneset.user_id == user_id
            else None
        )
        if (
            view is None
            and curation_view
            and geneweaverdb.user_is_assigned_curation(user_id, gs_id)
        ):
            view = "True"
    ...
    if not geneset:
        return render_template(
            "editgenesetsgenes.html", geneset=geneset, user_id=user_id
        )
...
def render_remove_genesets(gs_id):
    ...
    return render_template(
        "removegenesets.html", user_id=user_id, gs_and_proj=gs_and_proj
    )
...
def render_set_threshold(gs_id):
    ...
    if gsv_values is not None:
        for k in gsv_values:
            ...
            if maxVal is not None:
                maxVal = (
                    float(k_first_value) if float(k_first_value) > maxVal else maxVal
                )
            else:
            ...
            if minVal is not None:
                minVal = (
                    float(k_first_value) if float(k_first_value) < minVal else minVal
                )
            else:
    ...
    if threshold_type == 1 or threshold_type == 2:
        ...
        curr_gene_count = (
            curr_gene_count
            if thresh[0] == "None"
            else threshold_gene_counts[float(thresh[0])]
        )
    elif threshold_type == 4 or threshold_type == 5:
        curr_gene_count = calc_genes_in_threshold_range(
            gsv_values, thresh[0], thresh[1]
        )
    ...
    return render_template(
        ...
        gsv_values=g_values,
        threshold_gene_counts=threshold_gene_counts,
        curr_gene_count=curr_gene_count,
    )
    ...
    if gsv_values is not None:
        for k in gsv_values:
            ...
            d3BarChart.append(
                {
                    "x": i,
                    "y": float(k_first_value),
                    "gsid": str(list(k.values())[1]),
                    "abr": str(k_first_value),
                }
            )
            i += 1
...
def update_geneset_genes():
    if "user_id" in session:
        ...
        gs_id = request.args["gs_id"]
        if (
            geneweaverdb.get_user(user_id).is_admin != "False"
            or geneweaverdb.get_user(user_id).is_curator != "False"
        ...
        for gs in genesets:
            if not geneweaverdb.user_is_owner(user_id, gs.geneset_id):
                return jsonify(
                    success=False,
                    message=(
                        "You cannot share a private, Tier V GeneSet that "
                        "you do not own. Please remove the GeneSet from "
                        "this project before sharing it with others."
                    ),
                )

        if geneweaverdb.get_user(
            user_id
        ).is_admin != "False" or geneweaverdb.user_is_project_owner(user_id, proj_id):
...
def update_genesets_groups():
    ...
    if "user_id" in session:
        ...
        for product in itertools.product(gs_ids, groups):
            ...
            if geneweaverdb.get_geneset_tier(
                product[0]
            ) == 5 and not geneweaverdb.user_is_owner(user_id, product[0]):
        ...
        if not errors:
            ...
            return json.dumps(
                {
                    "error": (
                        "The GeneSet(s) %s are private (Tier V) and can "
        ...
        return json.dumps(
            {"error": "You must be logged in to share a geneset with a group"}
        )
...
def remove_users_from_group():
    ...
    results = geneweaverdb.remove_selected_users_from_group(
        user_id, user_emails, grp_id
    )
    return json.dumps(results)
...
@create_guest
def add_projects():
    results = geneweaverdb.add_project_by_name(
        request.args["name"], request.args["comment"]
    )
    return json.dumps(results)
...
def viewStoredResults_by_runhash():
    ...
    if results["res_tool"] == "Jaccard Similarity":
        return url_for(
            jaccardsimilarityblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )

    elif results["res_tool"] == "HiSim Graph":
        return url_for(
            phenomemapblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )

    elif results["res_tool"] == "GeneSet Graph":
        return url_for(
            genesetviewerblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )

    elif results["res_tool"] == "Clustering":
        return url_for(
            jaccardclusteringblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )

    elif results["res_tool"] == "Boolean Algebra":
        return url_for(
            booleanalgebrablueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )

    elif results["res_tool"] == "MSET":
        return url_for(msetblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash)

    elif results["res_tool"] == "UpSet":
        # TODO: Not sure that this upsetblueprint
        return url_for(upsetblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash)

    elif results["res_tool"] == "Find Variants":
        return url_for(
            findvariantsblueprint.TOOL_CLASSNAME + ".view_result", task_id=runhash
        )
...
def render_curategeneset(gs_id):
    ...
    if assignment:
        if request.args.get("reset_assignment_state") and (
            user.is_curator or user.is_admin
        ):
        ...
        if (
            assignment.state == curation_assignments.CurationAssignment.UNASSIGNED
            and user_is_curation_leader()
        ):
            ...
            assignment.state == curation_assignments.CurationAssignment.READY_FOR_REVIEW
            or assignment.state == curation_assignments.CurationAssignment.REVIEWED
        ):
        ...
        if curation_view:
            ...
            if curation_view == "curation_leader" or curation_view == "reviewer":
                ...
                curation_team_members = [
                    geneweaverdb.get_user(uid)
                    for uid in geneweaverdb.get_group_users(assignment.group)
                ]

            return render_viewgeneset_main(
                gs_id, curation_view, curation_team_members, assignment, curator_info
            )
...
def render_viewgeneset_main(
    gs_id,
    curation_view=None,
    curation_team=None,
    curation_assignment=None,
    curator_info=None,
):
    ...
    if user_info:
        if not user_info.is_admin and not user_info.is_curator:
            ...
            if not geneset and geneweaverdb.get_geneset_no_user(gs_id):
                return render_template(
                    "viewgenesetdetails.html", no_access=True, user_id=user_id
                )
    ...
    if threshold_type == 1 or threshold_type == 2:
        ...
        num_genes_in_threshold = (
            num_genes_in_threshold
            if curr_threshold[0] == "None"
            else gene_counts[float(curr_threshold[0])]
        )
    elif threshold_type == 4 or threshold_type == 5:
        if threshold == "," or len(curr_threshold) != 2:
            num_genes_in_threshold = numgenes
        elif len(curr_threshold) == 2:
            num_genes_in_threshold = calc_genes_in_threshold_range(
                gsv_values, curr_threshold[0], curr_threshold[1]
            )
    ...
    ## (like in classic GW)
    if (
        not user_info.is_admin
        and not user_info.is_curator
        and user_id != geneset.user_id
    ) and (not geneset or (geneset and geneset.status == "deleted")):
        return render_template(
            "viewgenesetdetails.html", geneset=None, deleted=True, user_id=user_id
        )
...
def render_viewgenesetoverlap(gs_ids):
    ...
    if len(list(set(gs_ids))) == 1:
        return render_template("viewgenesetoverlap.html", same_geneset=True)
    ...
    intersects = sorted(intersects, key=lambda i: i["intersect_count"], reverse=True)

    if user_id != 0:
        if user_info.is_admin or user_info.is_curator:
            view = "True"
        ...
        if i == j or ii == j:  # complete overlap
            ...
            while beta > 0:
                ...
                Sj = (
                    r1_2 * alpha
                    + r2_2 * (pi - beta)
                    - 0.5 * (r1_2 * math.sin(2 * alpha) + r2_2 * math.sin(2 * beta))
                )
                if Sj > j:
                    break
    ...
    if len(genesets) == 2:
        ...
        venn = venn_circles(gslen1, gslen2, len(intersects), 200)
        venn_text = {"tx": venn["tx"], "ty": venn["ty"]}
        venn = [
            {"cx": venn["c1x"], "cy": venn["c1y"], "r": venn["r1"]},
        ...
        if not group_id:
            if len(groups_owner) > 0:
                ...
                response = render_template(
                    "joinOrCreateGroups.html", groups=public_groups
                )
                return response
...
def group_id_in_groups(id, groups):
    ...
    for group in groups:
        if "grp_id" not in group:
            raise TypeError(
                "geneweaverdb.Groups instance has no grp_id value.  Possibly invalid type..."
            )
        if group["grp_id"] == id:
            included = True
    return included
...
def render_export_genelist(gs_id):
    if "user_id" in session:
        ...
        response.headers["Content-Disposition"] = (
            "attachment; filename=geneset_export.csv"
        )
        return response
...
def render_export_batch(gs_id):
    ...
    """
    title = (
        "gene_export_geneset_" + str(gs_id) + "_" + str(datetime.date.today()) + ".gw"
    )
    if "user_id" in session:
        import export_batch
...
def render_export_genes(gs_id):
    title = (
        "gene_export_geneset_" + str(gs_id) + "_" + str(datetime.date.today()) + ".txt"
    )
    if "user_id" in session:
...
def render_export_omicssoft(gs_ids):
    if "user_id" in session:
        ...
        for gs_id in gs_ids_list:
            ...
            print(omicssoft)
            title = (
                "gw_omicssoft_" + str(gs_id) + "_" + str(datetime.date.today()) + ".txt"
            )
            string += "[GeneSet]\n"
...
def HBA_batch_converter():
    ...
    if request.method == "POST":
        ...
        return render_export_hba(
            output, (header["GeneSet Abbreviation"]).replace(" ", "")
        )
...
def render_export_hba(txt, abbreviation):
    ...
    """
    title = (
        "HBA_batch_upload_" + abbreviation + "_" + str(datetime.date.today()) + ".txt"
    )
...
def render_view_same_publications(gs_id):
    ...
    return render_template(
        "viewsamepublications.html", user_id=user_id, gs_id=gs_id, geneset=results
    )
...
def render_searchFromHome():
    ...
    field_list = {
        "searchGenesets": False,
        "searchGenes": False,
        "searchAbstracts": False,
        "searchOntologies": False,
    }
    search_fields = list()
    ...
    search_values = search.keyword_paginated_search(
        terms, pagination_page, search_fields, default_filters, sortby
    )
    ...
    if search_values["STATUS"] == "NO MATCHES":
        return render_template("search.html", paginationValues=None, noResults=True)
...
def render_project_genesets():
    ...
    geneset_threshold_counts = geneweaverdb.get_genesets_with_threshold_counts(
        geneset_ids
    )
...
def get_project_groups_modal():
    ...
    return render_template("modal/viewProjectGroups.html", proj={"project_id": pid})
...
def get_add_project_group_modal():
    ...
    return render_template("modal/addProjectToGroup.html", proj=project)
    ...
    if "user" in flask.g and flask.g.user.is_admin:
        ...
        for tool in data:
            ...
            for key in data[tool]:
                temp.update(
                    {
                        str(key).split("-")[1] + "/" + str(key).split("-")[2]: data[
                            tool
                        ][key]
                    }
                )
    ...
    if "user" in flask.g and flask.g.user.is_admin:
        ...
        for t in tools:
            ...
            for item in data:
                if item["res_tool"] == tool:
                    ...
                    genesets_by_resid.update(
                        {item["res_id"]: item["gs_ids"].split(",")}
                    )
        ...
        for item in geneset_sizes:
            for num in geneset_sizes[item]:
                ...
                geneset_sizes[item][num] = {
                    "time": avg.total_seconds() * 1000,
                    "genes": avggenes,
                }
...
def get_assignment_status_string(status):
    status_text = {
        "status": curation_assignments.CurationAssignment.status_to_string(status)
    }
...
def add_genesets_projects():
    ...
    results = (
        {"error": "None", "is_guest": "True"}
        if flask.g.user.is_guest
        else {"error": "None"}
    )
...
def remove_non_ascii(text):
    return (
        "".join([i if ord(i) < 128 else " " for i in text.strip("\r")])
        if text
        else None
    )
...
def split_lines_and_tabs(to_split=""):
    return (
        (re.split(r"\t+", row) for row in iter(to_split.splitlines()))
        if to_split
        else ()
    )
...
def add_geneset_gene():
    ...
    new_genes = (
        input_data.get("text_data") + os.linesep + input_data.get("file_contents")
    )
    new_genes = split_lines_and_tabs(remove_non_ascii(new_genes))
    gene_dict = {
        split[0]: {"id": split[0], "value": split[1]}
        for split in new_genes
        if len(split) > 1
    }
    ...
    if not geneweaverdb.user_can_edit(user.user_id, gs_id) and not (
        user.is_admin or user.is_curator
    ):
    ...
    if len(gene_dict) < 1:
        ...
        missing = []
        gene_ids = geneweaverdb.geneset_gene_identifiers(
            gs_id, [(gene_dict[gene]["id"],) for gene in gene_dict]
        )
        ...
        warnings += [
            {"warning": "Identifier Not Found for ID: {}".format(gene)}
            for gene in missing
        ]
        ...
        if len(gene_dict) < 1:
            ...
            existing = geneweaverdb.existing_identifiers_for_geneset(
                gs_id, tuple(gene_dict[gene]["id"] for gene in gene_dict)
            )
            ...
            if overwrite:
                warnings += [
                    {
                        "warning": "The value for '{}', has been overwritten in this geneset".format(
                            gene
                        )
                    }
                ...
                warnings += [
                    {
                        "warning": "The Source ID '{}', already exists for this geneset".format(
                            gene
                        )
                    }
                    for gene in existing
            ...
            row_list = [
                (
                    gs_id,
                    gene_dict[gene]["ode_gene_id"],
                    gene_dict[gene]["value"],
                    gene_dict[gene]["id"],
                )
                for gene in gene_dict
            ]
            if row_list:
                db_results, db_errors = geneweaverdb.add_geneset_genes_to_temp(
                    gs_id, row_list
                )
                results += db_results
                errors += db_errors
...
def update_alternate_gene_symbol():
    ...
    # for gsv in gs.geneset_values:
    for gsv in gsvs:
        geneset_values.append(
            {
                "ode_gene_id": gsv.ode_gene_id,
...
def render_datasources():
    ...
    return render_template(
        "datasources.html", attributions=attlist, attcounts=attcounts
    )
...
def landing_page():
    ...
    sp_homology_count = {
        "Mus musculus": 100054,
        "Rattus norvegicus": 85593,
        "Saccharomyces cerevisiae": 31856,
        "Caenorhabditis elegans": 71921,
        "Drosophila melanogaster": 74710,
        "Danio rerio": 92720,
        "Homo sapiens": 101300,
    }
    ...
    return render_template(
        ...
        gs_counts=gs_counts,
        sp_counts=sp_counts,
        fly_gene_lengths=fly_gene_lengths,
        mouse_gene_lengths=mouse_gene_lengths,
        human_gene_lengths=human_gene_lengths,
        syms=syms,
        gene_ids=gene_ids,
        counts=counts,
        diseases=diseases,
        homolog_vals=list(sp_homology_count.values()),
...
def change_password():
    user = flask.g.user
    if (
        geneweaverdb.authenticate_user(user.email, request.form.get("curr_pass"))
        is None
    ):
        return "Fail"
    else:
...
def render_home():
    ...
    return render_template(
        "index.html", news_array=news_array, stats=stats, version=VERSION
    )
...
@login_required(allow_guests=True)
@app.route(
    "/add_geneset_to_project/<string:project_id>/<string:geneset_id>.html",
    methods=["GET", "POST"],
)
...
def message_group_json():
    ...
    if group_id in [g.grp_id for g in geneweaverdb.get_groups_owned_by_user(user_id)]:
        ...
        response = jsonify(
            success=False, message="You must be a group owner to message group members."
        )
        response.status_code = 401
...
def message_all_json():
    ...
    if geneweaverdb.get_user(user_id).is_admin:
        ...
        response = jsonify(
            success=False, message="You must be an admin to message all users."
        )
        response.status_code = 401
...
class GetGenesetsByGeneRefId(restful.Resource):
    def get(self, apikey, gene_ref_id, gdb_name):
        return geneweaverdb.get_genesets_by_gene_id(
            apikey, gene_ref_id, gdb_name, False
        )
...
class ToolJaccardClustering(restful.Resource):
    def get(self, apikey, homology, method, genesets):
        return jaccardclusteringblueprint.run_tool_api(
            apikey, homology, method, genesets
        )
...
class ToolGenesetViewer(restful.Resource):
    def get(self, apikey, homology, supressDisconnected, minDegree, genesets):
        return genesetviewerblueprint.run_tool_api(
            apikey, homology, supressDisconnected, minDegree, genesets
        )
...
class ToolJaccardSimilarity(restful.Resource):
    def get(self, apikey, homology, pairwiseDeletion, genesets):
        return jaccardsimilarityblueprint.run_tool_api(
            apikey, homology, pairwiseDeletion, genesets
        )
...
class ToolTricliqueViewer(restful.Resource):
    def get(self, apikey, homology, pairwiseDeletion, genesets):
        return triclique_viewer_blueprint.run_tool_api(
            apikey, homology, pairwiseDeletion, genesets
        )
...
class ToolPhenomeMap(restful.Resource):
    def get(
        ...
        return phenomemapblueprint.run_tool_api(
            apikey,
            homology,
            minGenes,
            permutationTimeLimit,
...
api.add_resource(
    KeywordSearchGuest, "/api/get/search/bykeyword/<apikey>/<search_term>/"
)

api.add_resource(
    GetGenesetsByGeneRefId,
    "/api/get/geneset/bygeneid/<apikey>/<gene_ref_id>/<gdb_name>/",
)
api.add_resource(
    GetGenesetsByGeneRefIdHomology,
...
api.add_resource(
    GetOntologyByGensetId, "/api/get/ontologies/bygeneset/<apikey>/<gs_id>/"
)
...
api.add_resource(
    GetPublicationById, "/api/get/publication/byid/<apikey>/<publicationid>/"
)
...
api.add_resource(
    GetGeneDatabaseById, "/api/get/genedatabase/byid/<apikey>/<gene_database_id>/"
)
...
api.add_resource(
    AddGenesetToProject, "/api/add/geneset/toproject/<apikey>/<projectid>/<genesetid>/"
)
api.add_resource(
    DeleteGenesetFromProject,
...
api.add_resource(
    ToolCombineProjects, "/api/tool/combine/byprojects/<apikey>/<homology>/<projects>/"
)
...
api.add_resource(
    ToolBooleanAlgebra, "/api/tool/booleanalgebra/<apikey>/<relation>/<genesets>/"
)
api.add_resource(
    ToolBooleanAlgebraProjects,
...
api.add_resource(ToolUpSet, "/api/tool/upset/<apikey>/<homology>/<zeros>/<genesets>/")
api.add_resource(
    ToolUpSetProjects,
...
api.add_resource(
    ToolFindVariants, "/api/tool/findvariants/<apikey>/<species>/<genesets>/<path>/"
)
