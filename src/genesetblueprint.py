...
def create_batch_geneset():
    ...
    curation_note = "Geneset created in batch by {} {}".format(
        user.first_name, user.last_name
    )
    for gs_id in new_ids:
        curation_assignments.submit_geneset_for_curation(
            gs_id, curation_group, curation_note
        )

    return flask.jsonify(
        {"genesets": new_ids, "warn": batch_reader.warns, "error": batch_reader.errors}
    )
...
def infer_id_kind():
    ...
    with geneweaverdb.PooledCursor() as cursor:
        ...
        def add_counts(curr_id, use_gene_table):
            cursor.execute(
                gene_table_sql if use_gene_table else probe_table_sql,
                (curr_id.lower(),),
            )
    ...
    for id_kind_tuple, success_id_set in id_kind_mapping_dict.items():
        ...
        def item_as_dict():
            return {
                ...
                "source_id": "gene_{0}".format(source_id)
                if is_gene_result
                else "ma_{0}".format(source_id),
                "species_id": sp_id,
...
def create_temp_geneset():
    ...
    try:
                ...
                try:
                    ...
                    with geneweaverdb.PooledCursor() as cursor:
                        ...
                        if platform_results:
                            ...
                            # adds to geneID list if unique
                            if (
                                platform_results[0]["ode_gene_id"]
                                not in unique_gene_ids
                            ):
                                unique_gene_ids.append(
                                    platform_results[0]["ode_gene_id"]
                                )
        ...
        if len(invalid_genes) > 0:
            return (
                "Unable to find these Genes for specified species:\n"
                + ", ".join(invalid_genes)
        ...
        if form["pub_pubmed"] != None and form["pub_pubmed"] != "":
            ...
            with geneweaverdb.PooledCursor() as cursor:
                pubcheck = None
                cursor.execute(
                    """SELECT pub_id FROM production.publication where pub_pubmed=%s;""",
                    (form["pub_pubmed"],),
                )
                try:
            ...
            if exists == False:
                ...
                if len(cols) > 0:
                    ...
                    with geneweaverdb.PooledCursor() as cursor:
                        pub_sql = (
                            """INSERT INTO production.publication(%s) VALUES ('%s') RETURNING pub_id;"""
                            % (
                                ",".join(keys),
                                "','".join(values),
                            )
        ...
        with geneweaverdb.PooledCursor() as cursor:
            file_sql = (
                """INSERT INTO production.file(file_size, file_contents) VALUES (%s, '%s') RETURNING file_id;"""
                % (
                    len(file_text),
                    file_text,
                )
        ...
        with geneweaverdb.PooledCursor() as cursor:
            GS_sql = (
                """INSERT INTO production.geneset(gs_name, gs_description, gs_abbreviation, sp_id, usr_id, gs_created, cur_id, file_id, gs_status, gs_count) VALUES ('%s','%s','%s','%s',%s,now(),%s,%s,'%s',%s) RETURNING gs_id;"""
                % (
                    gs_name,
                    gs_description,
                    gs_abbreviation,
            ...
            if pub_id:
                pub_sql = (
                    """UPDATE production.geneset SET pub_id=%s WHERE gs_id=%s;"""
                    % (pub_id, gs_id)
                )
                cursor.execute(pub_sql)
                cursor.connection.commit()
        ...
        for ode_gene_id in unique_gene_ids:
            ...
            GS_value_sql = (
                """INSERT INTO extsrc.geneset_value(gs_id, ode_gene_id, gsv_value, gsv_source_list, gsv_value_list, gsv_hits, gsv_in_threshold) VALUES (%s,%s,'%s','{"%s"}','{%s}',%s,%s);"""
                % (
                    gs_id,
                    ode_gene_id,
                    avg,
        ...
        with geneweaverdb.PooledCursor() as cursor:
            gs_count_sql = (
                """SELECT count(ode_ref_id) FROM extsrc.geneset_value NATURAL JOIN extsrc.gene WHERE ode_pref AND gs_id=%s GROUP BY gs_id;"""
                % (gs_id)
            )
        ...
        with geneweaverdb.PooledCursor() as cursor:
            gs_update_sql = (
                """UPDATE production.geneset SET gs_count=%s, gs_threshold='%s', gs_threshold_type=%s WHERE gs_id=%s;"""
