# file: genesetblueprint.py
# authors: Tim Reynolds, Astrid Moore
...
import urllib2
import re
from batch import Batch, GeneSet
...
def render_uploadgeneset(genes=None):
	...
	gidts = []
	for gdb_name, gdb_id in uploader.get_geneTypes().iteritems():
		gidts.append(('gene_{0}'.format(gdb_id), gdb_name))

	microarray_sources = []
	for pf_name, pf_id in uploader.get_microArrayTypes().iteritems():
...
def create_batch_geneset():
	""" Attempts to parse a batch file and create a temporary GeneSet
		for review.
	"""
...
def create_temp_geneset():
	...
	# try to load the form (NOTE: add statement after testing)
	user_id = flask.session['user_id'] if 'user_id' in flask.session else 0
	form = flask.request.form
	file_text = form['file_text']
	file_lines = file_text.splitlines()
	gs_name = form['gs_name']
	abbrev = form['gs_abbreviation']
	description = form['gs_description']
	privacy = form['permissions']
	species = form['species']
	gs_gene_id_type = form['gene_identifier']  # ??
	# see what is in the file_text

	# if load was successful, pass file lines along
	batch = Batch(remote=True)
	temp_gs =


	# NOTE: test batch obj

	# grab any errors / warnings
	...
	# if load was successful, pass file_lines (Batch will upload GS)
	batch = Batch(file_list=file_lines)

	# NOTE: test batch obj
	# = add a global param to Batch to indicate a solo GeneSet input
	# - adapt assess_inputs()
	# - adapt create_batches()
	# - adapt upload_genesets()
	...
	# if there were warnings, report
	gs_names = batch.report_gs_names()  # grab abbrev geneset names
