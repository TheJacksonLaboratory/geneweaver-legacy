...
def db_conn():
    ...
    try:
        conn = psycopg2.connect(
            ("host='ode-db4.jax.org' dbname='ODE' user='odeadmin' password=''")
        )
    except:
...
def get_number_tier_3(conn):
    ...
    """
    conn.execute(
        """SELECT count(gs_id) FROM production.geneset WHERE gs_status NOT LIKE 'de%%' AND cur_id=3"""
    )
...
def geneinfo_batch(conn, gs_id):
    ...
    with conn as cursor:
        cursor.execute(
            """SELECT gs_gene_id_type FROM geneset WHERE gs_id=%s""", (gs_id,)
        )
    ...
    with conn as cursor:
        if gdb_id < 0:
            gdb_id *= -1
            sql = (
                "SELECT g.ode_ref_id, gsv.gsv_value "
        ...
        # Microarrays
            ...
            sql = (
                "SELECT prb.prb_ref_id, gsv.gsv_value "
