...
SRP_EFETCH_URL = (
    "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=sra&retttype=acc&id=%s"
)
...
def get_pubmed_info(pub_med_id):
    ...
    if resp.status_code == 200:
        ...
        if author_list_node is not None:
            authors = ", ".join(
                map(auth_node_to_str, author_list_node.findall(".//Author"))
            )
            if authors:
                try:
        ...
        raise Exception(
            "unexpected response status code from pubmed service: {0}".format(
                resp.status_code
            )
        )
