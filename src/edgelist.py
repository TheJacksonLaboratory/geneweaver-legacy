        ...
        with PooledCursor() as cursor:
            cursor.execute(
                """SELECT gv.ode_gene_id FROM geneset_value gv, project2geneset p2g
                              ...
                              INTERSECT
                              SELECT gv.ode_gene_id FROM geneset_value gv, project2geneset p2g
                              WHERE p2g.pj_id=1430 and p2g.gs_id=gv.gs_id""",
                (
                    proj1,
                    proj2,
                ),
...
def intersect_geneset(gsid1, gsid2, hom=True):
    ...
    if not hom:
        with PooledCursor() as cursor:
            cursor.execute(
                """SELECT ode_gene_id FROM geneset_value WHERE gs_id=%s
                              INTERSECT
                              SELECT ode_gene_id FROM geneset_value WHERE gs_id=%s""",
                (
                    gsid1,
...
def get_jaccard(pjid1, pjid2, threshold):
    ...
    with PooledCursor() as cursor:
        cursor.execute(
            """SELECT jac_value FROM geneset_jaccard WHERE gs_id_left=%s AND gs_id_right=%s AND
                          jac_value > %s""",
            (
                pj1,
                pj2,
                ...
                for m in genesets[projs[i]]:
                    for n in genesets[projs[k]]:
                        ...
                        if jac_value > 0:
                            ...
                            if (
                                str(m.geneset_id) not in fileout
                                and str(m.geneset_id) not in ignored
                            ):
                                ignored.append(str(m.geneset_id))
                                ign_projects.append(str(projs[i]))
                            if (
                                str(n.geneset_id) not in fileout
                                and str(n.geneset_id) not in ignored
                            ):
...
def create_kpartite_file_from_gene_intersection(
    taskid, results, proj1, proj2, homology
):
    """
    ...
    if len(genes) > 0:
        ...
        partition3 = edge_proj2proj(
            [v.geneset_id for v in projDict1], [v.geneset_id for v in projDict2]
        )

        # Create the first line (1st part, 2nd part, 3rd part, total)
        # and write out to RESULTS directory
        firstLine = (
            str(len(partition1) + len(partition3))
            + "\t"
            + str(len(partition2) + len(partition3))
            + "\t"
            + str(len(partition1) + len(partition2))
            + "\t"
            + str(len(partition1) + len(partition2) + len(partition3))
            + "\n"
        )
...
def get_matrix_value(i, j, identifiers, partitions):
    ...
    if len(set(id2Found).intersection(id1Found)) > 0:
        ...
        with PooledCursor() as cursor:
            cursor.execute(
                cursor.mogrify(
                    "SELECT gs_count FROM geneset WHERE gs_id =' " + geneset_id + " ' "
                )
            )
...
def create_csv_from_mkc(taskid, results, identifiers, partitions):
    ...
    for p in range(len(gs_ids)):
        with PooledCursor() as cursor:
            cursor.execute(
                cursor.mogrify(
                    "SELECT gs_name FROM geneset WHERE gs_id =' " + gs_ids[p] + " ' "
                )
            )
    ...
    for g in range(len(gi_ids)):
        with PooledCursor() as cursor:
            cursor.execute(
                cursor.mogrify(
                    "SELECT ode_ref_id FROM gene WHERE ode_pref='t' and gdb_id=7 and ode_gene_id = ' "
                    + gi_ids[g]
                    + " ' "
                )
            )
    ...
    for g in range(len(gs_ids)):
        with PooledCursor() as cursor:
            cursor.execute(
                cursor.mogrify(
                    "SELECT gs_abbreviation FROM geneset WHERE gs_id = ' "
                    + gs_ids[g]
                    + " ' "
                )
            )
    ...
    for i in range(len(partitions)):
        for j in range(len(partitions[i])):
            f.write(
                str(identifiers[count])
                + ","
                + g_names[count]
                + ","
                + g_abbr[count]
                + ",0,"
                + HOMOLOGY_BOX_COLORS[i]
                + "\n"
            )
            print(
                str(identifiers[count])
...
def create_json_from_triclique_output_jaccard(taskid, results):
    ...
    for i in range(n):
        for j in range(n):
            Matrix[i][j] = get_matrix_value_jaccard(
                i, j, identifiers, partitions, ignored
            )
...
def create_csv_from_mkc_jaccard(taskid, results, identifiers, partitions):
    ...
    for p in range(len(genesets)):
        with PooledCursor() as cursor:
            cursor.execute(
                cursor.mogrify(
                    "SELECT gs_name FROM geneset WHERE gs_id =' " + genesets[p] + " ' "
                )
            )
...
def get_matrix_value_jaccard(i, j, identifiers, partitions, ignored):
    ...
    if id1Found == id2Found:
        ...
        with PooledCursor() as cursor:
            cursor.execute(
                """SELECT jac_value FROM geneset_jaccard WHERE gs_id_left=%s AND gs_id_right=%s""",
                (
                    gs_id1,
                    gs_id2,
                ),
