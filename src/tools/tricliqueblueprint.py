...
def run_tool():
    ...
    if params2[method_str] == "ExactGeneOverlap":
        ...
        outOfBounds = create_kpartite_file_from_gene_intersection(
            task_id,
            RESULTS_PATH,
            selected_project_ids[0],
        ...
        if len(selected_geneset_ids) > 50:
            flask.flash(
                "Warning: Selecting too many genesets will not return results in a timely fashion"
            )
            return flask.redirect("/analyze")
        outOfBounds = create_kpartite_file_from_jaccard_overlap(
            task_id,
            RESULTS_PATH,
            selected_project_ids,
            thresholds["Triclique_ThresholdValues"],
        )

    if outOfBounds == -1:
        flask.flash(
            "Warning: The genesets for the projects you chose had no intersection"
        )
        return flask.redirect("/analyze")
    if outOfBounds == -2:
...
@triclique_viewer_blueprint.route(
...
def view_result(task_id):
    ...
    if async_result.state in states.PROPAGATE_STATES:
        ...
        if os.path.isfile(jfile_name):
            triclique_result = create_json_from_triclique_output_jaccard(
                task_id, RESULTS_PATH
            )
        if triclique_result == 1:
            flask.flash(
                "Warning: The genesets for the projects you chose had no maximal triclique"
            )
            return flask.redirect("/analyze")
