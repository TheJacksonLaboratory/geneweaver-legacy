...
def geneset_intersection(gsID_1, gsID_2, i):
    ...
    if user_id:
        ...
        for j in range(0, len(temp_genes[0])):
            intersect_genes[temp_genes[0][j]] = gwdb.if_gene_has_homology(
                temp_genes[1][j]
            )
        list = gwdb.get_all_projects(user_id)
    else:
    ...
    return flask.render_template(
        "geneset_intersection.html",
        async_result=json.loads(async_result.result),
        index=i,
        genesets=genesets,
        gene_sym=intersect_genes,
        list=list,
    )
