...
def run_tool():
    ...
    if len(selected_geneset_ids) < 2:
        flask.flash(("You need to select at least 2 genesets as input for this tool."))

        return flask.redirect("/analyze")

    if "BooleanAlgebra_Relation" not in form:
        flask.flash(("You need to select a boolean algebra relation for this tool."))
...
def get_results(async_result, tool):
    if async_result.state in states.PROPAGATE_STATES:
        ...
        if results["error"]:
            ...
            raise BooleanResultError(
                "An unknown error occurred. Please contact a GeneWeaver admin."
            )
...
def get_emphgenes(user_id):
    return [
        str(row["ode_gene_id"])
        for row in gwdb.get_gene_and_species_info_by_user(user_id)
    ]
...
def paginate_bool_dicts(json_results, page, per_page):
    if "bool_results" in json_results:
        json_results["bool_results"] = paginate_dict(
            json_results["bool_results"], page, per_page
        )
    if "bool_except" in json_results:
        json_results["bool_except"] = {
            k: paginate_dict(v, page, per_page)
            for k, v in json_results["bool_except"].items()
        }
    if "intersect_results" in json_results:
        json_results["intersect_results"] = {
            k: paginate_dict(v, page, per_page)
...
@boolean_algebra_blueprint.route(
...
@boolean_algebra_blueprint.route(
    "/" + TOOL_CLASSNAME + "-result/<task_id>/output.json", methods=["GET"]
)
